<script lang="ts">
	...
	// Images
	import HeroDarkImg from '$lib/imgs/code.png';
	import HeroLightImg from '$lib/imgs/code.png';
	let inView = false;
...
<section id="hero" class="relative mx-auto mt-32 max-w-7xl px-6 text-center md:px-8">
	...
	>
		Beautifully designed, And Fasted
		<br class="hidden md:block" />
		{' '}
		In Terminal.
  ...
  class="relative mt-32 animate-fade-up opacity-0 [--animation-delay:400ms] [perspective:2000px] after:absolute after:inset-0 after:z-50 after:[background:linear-gradient(to_top,hsl(var(--background))_30%,transparent)] shadow-[0_4px_20px_rgba((189, 195, 199);, 1)]"
>
  <div
    class="rounded-xl border border-white/10 bg-[#2e2e2e] bg-opacity-[0.01] before:absolute before:bottom-1/2 before:left-0 before:top-0 before:size-full before:opacity-0 before:[background-image:linear-gradient(to_bottom,#a3a3a3,#a3a3a3,transparent_40%)] before:[filter:blur(180px)] {inView
